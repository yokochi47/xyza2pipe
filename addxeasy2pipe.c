...
static const char *usage[] = {
		"addxeasy2pipe -i/--in1 inTemplate1(2D/3D/4D) -j/--in2 inTemplate2(2D/3D/4D) > stdout\n", "Optional Arguments:\n",
...
int main(int argc, char *argv[])
{
	...
	if (checkxeasy(filename2) != 0)
		return EXIT_FAILURE;

	if (dimension != _dimension) {
		fprintf(stderr, "addxeasy2pipe error: unmatch dimension number.\n");
		return EXIT_FAILURE;
	}

	for (l = 0; l < dimension; l++) {
		if (datasize[l] != datasize_orig[l]) {
			fprintf(stderr, "addxeasy2pipe error: unmatch data size.\n");
			return EXIT_FAILURE;
		}
	}
	...
	if (isatty(STDOUT_FILENO)) {
		fprintf(stderr, "addxeasy2pipe error: output to terminal.\n");
		return EXIT_FAILURE;
	}

	switch (dimension) {
	case 2:
		return pushaddxeasy2d(filename1, filename2, c1, c2, opr_code);
	case 3:
		return pushaddxeasy3d(filename1, filename2, c1, c2, opr_code);
	case 4:
		return pushaddxeasy4d(filename1, filename2, c1, c2, opr_code);
	default:
		fprintf(stderr, "addxeasy2pipe error: unsupported dimension number.\n");
		return EXIT_FAILURE;
	}
