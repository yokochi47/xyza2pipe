...
extern float xeasy2float(unsigned char *x16);
...
int openxeasy2d(char spectra2d[], float **mat2d)
{
	...
	for (j = 0; j < datasize[1]; j++) {
		...
		for (i = 0; i < datasize[0]; i++) {
			...
			fread(x16, sizeof(char), sizeof(short), fp);

			mat2d[j][i] = xeasy2float(x16);
		}
	}
...
int openxeasy3d(char spectra3d[], float ***mat3d)
{
	...
	/* CHECK FILE SIZE */
	if (size != sizeof(short) * datasize_3d) {
		fprintf(stderr, "Spectra file %s: Partially broken. (Actual=%d Expected=%d)\n", spectra3d, (int) (size),
				(int) (sizeof(short)) * datasize_3d);
		goto escape;
	}
...
int openxeasy4d(char spectra4d[], float ****mat4d)
{
	...
	/* CHECK FILE SIZE */
	if (size != sizeof(short) * datasize_4d) {
		fprintf(stderr, "Spectra file %s: Partially broken. (Actual=%d Expected=%d)\n", spectra4d, (int) (size),
				(int) (sizeof(short)) * datasize_4d);
		goto escape;
	}
