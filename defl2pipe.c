...
static const char *usage[] = {
		"defl2pipe -i/--in inTemplate(2D/3D/4D) > stdout\n", "Optional Arguments:\n", " -x  Output X-Vector first\n",
...
int main(int argc, char *argv[])
{
	...
	if (checkdefl(filename) != 0)
		return EXIT_FAILURE;

	cnvhdr(axis_option, 'f');

	if (isatty(STDOUT_FILENO)) {
		fprintf(stderr, "defl2pipe error: output to terminal.\n");
		return EXIT_FAILURE;
	}

	switch (dimension) {
	case 2:
		return pushxyza2d(filename, axis_option);
	case 3:
		return pushxyza3d(filename, axis_option);
	case 4:
		return pushxyza4d(filename, axis_option);
	default:
		fprintf(stderr, "defl2pipe error: unsupported dimension number.\n");
		return EXIT_FAILURE;
	}
