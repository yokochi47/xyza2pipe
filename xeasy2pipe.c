...
static const char *usage[] = {
		"xeasy2pipe -i/--in inTemplate(2D/3D/4D) > stdout\n", "Optional Arguments:\n", " -x  Output X-Vector first\n",
...
int main(int argc, char *argv[])
{
	...
	while (1) {
		l = getopt_long(argc, argv, "i:xyza", options, &opt_idx);
	...
	if (checkxeasy(filename) != 0)
		return EXIT_FAILURE;

	cnvhdr(axis_option, 'f');

	if (isatty(STDOUT_FILENO)) {
		fprintf(stderr, "xeasy2pipe error: output to terminal.\n");
		return EXIT_FAILURE;
	}

	switch (dimension) {
	case 2:
		return pushxeasy2d(filename, axis_option);
	case 3:
		return pushxeasy3d(filename, axis_option);
	case 4:
		return pushxeasy4d(filename, axis_option);
	default:
		fprintf(stderr, "xeasy2pipe error: unsupported dimension number.\n");
		return EXIT_FAILURE;
	}
