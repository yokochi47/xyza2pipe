...
static const char *usage[] = {
		"pipe2azara -o/--out outTemplate(2D/3D/4D) < stdin\n", "Optional Arguments:\n", " -s/--swap  Byteswap data\n",
...
int main(int argc, char *argv[])
{
	...
	while (1) {
		l = getopt_long(argc, argv, "o:slg", options, &opt_idx);
	...
	switch (dimension) {
	case 2:
		if (pullazara2d(filename) != 0)
			return EXIT_FAILURE;

		break;
	case 3:
		if (pullazara3d(filename) != 0)
			return EXIT_FAILURE;

		break;
	case 4:
		if (pullazara4d(filename) != 0)
			return EXIT_FAILURE;

		break;
	}

	return checkazara(filename);
}
