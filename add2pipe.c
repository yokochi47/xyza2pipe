...
int main(int argc, char *argv[])
{
	...
	if (checkxyza(filename2) != 0)
		return EXIT_FAILURE;

	if (dimension != _dimension) {
		fprintf(stderr, "add2pipe error: unmatch dimension number.\n");
		return EXIT_FAILURE;
	}

	for (l = 0; l < dimension; l++) {
		if (datasize[l] != datasize_orig[l]) {
			fprintf(stderr, "add2pipe error: unmatch data size.\n");
			return EXIT_FAILURE;
		}
	}
	...
	if (isatty(STDOUT_FILENO)) {
		fprintf(stderr, "add2pipe error: output to terminal.\n");
		return EXIT_FAILURE;
	}

	switch (dimension) {
	case 2:
		return pushadd2d(filename1, filename2, c1, c2, opr_code);
	case 3:
		return pushadd3d(filename1, filename2, c1, c2, opr_code);
	case 4:
		return pushadd4d(filename1, filename2, c1, c2, opr_code);
	default:
		fprintf(stderr, "add2pipe error: unsupported dimension number.\n");
		return EXIT_FAILURE;
	}
