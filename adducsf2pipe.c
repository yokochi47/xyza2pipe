...
static const char *usage[] = {
		"adducsf2pipe -i/--in1 inTemplate1(2D/3D/4D) -j/--in2 inTemplate2(2D/3D/4D) > stdout\n", "Optional Arguments:\n",
...
int main(int argc, char *argv[])
{
	...
	if (checkucsf(filename2) != 0)
		return EXIT_FAILURE;

	if (dimension != _dimension) {
		fprintf(stderr, "adducsf2pipe error: unmatch dimension number.\n");
		return EXIT_FAILURE;
	}

	for (l = 0; l < dimension; l++) {
		if (datasize[l] != datasize_orig[l]) {
			fprintf(stderr, "adducsf2pipe error: unmatch data size.\n");
			return EXIT_FAILURE;
		}
	}
	...
	if (isatty(STDOUT_FILENO)) {
		fprintf(stderr, "adducsf2pipe error: output to terminal.\n");
		return EXIT_FAILURE;
	}

	switch (dimension) {
	case 2:
		return pushadducsf2d(filename1, filename2, c1, c2, opr_code);
	case 3:
		return pushadducsf3d(filename1, filename2, c1, c2, opr_code);
	case 4:
		return pushadducsf4d(filename1, filename2, c1, c2, opr_code);
	default:
		fprintf(stderr, "adducsf2pipe error: unsupported dimension number.\n");
		return EXIT_FAILURE;
	}
