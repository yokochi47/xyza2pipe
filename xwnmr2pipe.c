...
static const char *usage[] = {
		"xwnmr2pipe -i/--in inTemplate(2D/3D/4D) > stdout\n", " -e/--extLeft Extract left hand of X-Axis.\n",
...
int main(int argc, char *argv[])
{
	...
	while (1) {
		l = getopt_long(argc, argv, "i:xyzaed:", options, &opt_idx);
	...
	if (checkxwnmr(filename) != 0)
		return EXIT_FAILURE;

	cnvhdr(axis_option, 'f');

	if (isatty(STDOUT_FILENO)) {
		fprintf(stderr, "xwnmr2pipe error: output to terminal.\n");
		return EXIT_FAILURE;
	}

	switch (dimension) {
	case 2:
		return pushxwnmr2d(filename, axis_option);
	case 3:
		return pushxwnmr3d(filename, axis_option);
	case 4:
		return pushxwnmr4d(filename, axis_option);
	default:
		fprintf(stderr, "xwnmr2pipe error: unsupported dimension number.\n");
		return EXIT_FAILURE;
	}
