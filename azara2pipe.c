...
static const char *usage[] = {
		"azara2pipe -i/--in inTemplate(2D/3D/4D) > stdout\n", "Optional Arguments:\n", " -x  Output X-Vector first\n",
...
int main(int argc, char *argv[])
{
	...
	if (checkazara(filename) != 0)
		return EXIT_FAILURE;

	cnvhdr(axis_option, 'f');

	if (isatty(STDOUT_FILENO)) {
		fprintf(stderr, "azara2pipe error: output to terminal.\n");
		return EXIT_FAILURE;
	}

	switch (dimension) {
	case 2:
		return pushazara2d(filename, axis_option);
	case 3:
		return pushazara3d(filename, axis_option);
	case 4:
		return pushazara4d(filename, axis_option);
	default:
		fprintf(stderr, "azara2pipe error: unsupported dimension number.\n");
		return EXIT_FAILURE;
	}
